...
describe("video utils", () => {
  ...
  describe("cas edge (valeurs nulles / URL sans extension)", () => {
    it("extractServerId retourne null pour chaîne vide, undefined et valeur non-string", () => {
      expect(extractServerId("")).toBe(null);
      expect(extractServerId(undefined as unknown as string)).toBe(null);
      expect(extractServerId(123 as unknown as string)).toBe(null);
    });

    it("analyzeVideo retourne {url, valid:false} sans qualité ni serveur pour chaîne vide/undefined", () => {
      const empty = analyzeVideo("");
      expect(empty.valid).toBe(false);
      expect(empty.quality).toBeUndefined();
      expect(empty.serverId).toBeUndefined();

      const undef = analyzeVideo(undefined as unknown as string);
      expect(undef.valid).toBe(false);
      expect(undef.quality).toBeUndefined();
      expect(undef.serverId).toBeUndefined();
    });

    it("isValidVideoUrl retourne false pour une URL sans extension", () => {
      // Pas d'extension reconnue -> pas de match -> false (chemin !match).
      expect(isValidVideoUrl("https://cdn.test/video")).toBe(false);
    });
  });
});
