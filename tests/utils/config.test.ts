...
describe('config', () => {
  ...
  it('isTmdbConfigured détecte la clé', () => {
    expect(isTmdbConfigured({ tmdbApiKey: '', meilisearchHost: '', meilisearchMasterKey: '', maxConcurrency: 1, requestDelayMin: 1, requestDelayMax: 1, logLevel: 'info' })).toBe(false);
    expect(isTmdbConfigured({ tmdbApiKey: 'k', meilisearchHost: '', meilisearchMasterKey: '', maxConcurrency: 1, requestDelayMin: 1, requestDelayMax: 1, logLevel: 'info' })).toBe(true);
  });
});
